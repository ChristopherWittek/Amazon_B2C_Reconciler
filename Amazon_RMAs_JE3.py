...
if len(red_deltas_df) > 0 :
    for key,val in red_deltas_df.iterrows() :
        ...
        elif abs(val['Delta']) / 2 == abs(val['Grand Total']) or val['Grand Total'] == 0 or val['Principal'] == 0 or (val['Grand Total'] < 0 and val['Credit'] == 0) :
...
if len(so_missing_rma) > 0 :
    ...
    netsuite_orders_rma = pd.read_csv(path_parent+r'\NetSuite Data\NetSuite_Amazon_orders.csv', delimiter=',',encoding='iso8859_15')
    # netsuite_orders_rma = netsuite_orders_rma.rename(columns={'ChannelAdvisor Order ID':'order-id',netsuite_orders_rma.columns[1]:'Internal ID'})

    netsuite_orders_rma = netsuite_orders_rma.iloc[:,[0,3,4,7]].rename(columns={'ChannelAdvisor Order ID':'order-id',netsuite_orders_rma.columns[0]:'Internal ID'})
    netsuite_orders_rma.to_csv(recon_path_data+r'\netsuite_orders_rma.csv')
...
try :
    ...
    manual_rma_entry = manual_rma_entry[['Internal ID','order-id','Grand Total', 'Sale', 'Credit', 'Delta']].rename(columns={'Internal ID':'Sales Order : Internal ID','order-id':'ChannelAdvisor oder-id','Grand Total':'Amazon Statement','Sale':'NS Payment','Credit':'NS Refund'}).drop_duplicates('ChannelAdvisor oder-id')
